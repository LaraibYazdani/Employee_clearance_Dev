...
export async function notifySection2Approvers(clearanceId: string): Promise<void> {
  ...
  for (const section of clearance.clearance_sections) {
    if (!section2Keys.includes(section.section_key)) continue
    if (!section.approver_id) continue

    const approver = await prisma.user.findUnique({ where: { id: section.approver_id } })
    if (!approver) continue

    const sectionLabel = SECTION_LABELS[section.section_key] ?? section.section_key
...
export async function notifySection3Approvers(clearanceId: string): Promise<void> {
  ...
  for (const section of clearance.clearance_sections) {
    if (!section3Keys.includes(section.section_key)) continue
    if (!section.approver_id) continue
...
/**
 * Notifies the relevant section approver that their section has been re-routed by the HRBP.
 */
export async function notifyApproverRerouted(
  ...
  const section = clearance.clearance_sections[0]
  if (!section || !section.approver_id) return
